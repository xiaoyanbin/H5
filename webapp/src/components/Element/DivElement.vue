<template>
  <div class='wrap' @mousedown="mousedown" @mouseup="mouseup">
     <div style="width:100%;height:100%; background-size:100%; background-repeat:no-repeat"  v-bind:style="{backgroundImage: 'url(' + http + element.imgSrc + ')'}">  
     </div>
...
<script>
    ...
    export default{
      ...
      methods: {
        styles(){
             return {'background-image': "url("+this.http + this.element.imgSrc+")",'background-size':'100%'};
        }, 
        // 处理元素拖动
        move () {
          ...
          document.querySelector('.editor').onmousemove = (event) => {
            ...
            if (this.scaleFlag) {
              ...
              switch (this.direction) {
                ...
                case 'se':
                  this.element.height = parseInt(this.height) + disY
                  this.element.width = parseInt(this.width) + disY*this.width/this.height
                  break
              }
            }
          }
